...
WHITE = (255, 255, 255)
LINE_COLOR = (50, 50, 50)
FOOD_COLOR = (219, 20, 159)
FOOD_COLOR_2 = (232, 21, 31)
HEIGHT = 400
WIDTH = 400
SCORE = 0
BLOCK_SIZE = 20
...
class Food:
    ...
    def __init__(self, wall):
        self.location = Point(choice([i for i in range(1,19) if i != 10 and i not in wall.body]), choice([i for i in range(1,19) if i != 11 and i not in wall.body]))

    def draw(self):
        point = self.location
        rect = pygame.Rect(BLOCK_SIZE * point.x, BLOCK_SIZE * point.y, BLOCK_SIZE, BLOCK_SIZE)
        pygame.draw.rect(SCREEN, FOOD_COLOR_2, rect)
...
class Extra_Food:
    ...
    def draw(self):
        ...
        if self.cnt == 5:
            ...
            if collision_1:
                self.cnt = 0
                self.location = Point(choice([i for i in range(1,19) if i != 10]), choice([i for i in range(1,19) if i != 11]))
                collision_1 = False
...
class Snake:
    ...
    def draw(self):
        for point in self.body[1:]:
            rect = pygame.Rect(BLOCK_SIZE * point.x, BLOCK_SIZE * point.y, BLOCK_SIZE, BLOCK_SIZE)
            pygame.draw.rect(SCREEN, (22, 92, 22), rect)

        point = self.body[0]
        rect = pygame.Rect(BLOCK_SIZE * point.x, BLOCK_SIZE * point.y, BLOCK_SIZE, BLOCK_SIZE)
        pygame.draw.rect(SCREEN, (0, 64, 0), rect)

    def check_collision(self, food, food_1):
        global SCORE, lifespan
    ...
    def check_collision_1(self, food_1):
        global SCORE, collision_1
        if self.body[0].x == food_1.location.x:
            if self.body[0].y == food_1.location.y:
                collision_1 = True
                SCORE += 3
...
def main():
    ...
    while True:
        ...
        snake.move()
        snake.check_collision(food, food_1)
        snake.check_collision_1(food_1)
